...
# Text area for pasting encrypted text
text_input = st.text_area("Or paste your encrypted text here:", height=200)
...
if uploaded_file is not None:
    ...
    if st.session_state.encrypted_text is None:
        ...
        st.write("### Encrypted Text from File:")
        st.text_area("Encrypted Text", st.session_state.encrypted_text, height=200)
else:
    # If text is pasted, use that instead
    if text_input:
        st.session_state.encrypted_text = text_input.upper()  # Store as uppercase for consistency
        st.write("### Encrypted Text from Input:")
        st.text_area("Encrypted Text", st.session_state.encrypted_text, height=200)

# Set decrypted text if not already set
if not st.session_state.decrypted_text and st.session_state.encrypted_text:
...
with col3:
    if st.button("Apply Substitution"):
        if old_letter and new_letter:
            ...
            st.session_state.decrypted_text = substitute_letters(st.session_state.decrypted_text, old_letter, new_letter)
